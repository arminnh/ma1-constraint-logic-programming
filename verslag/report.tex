...
Our Hashiwokakero solver in ECLiPSe is based on the stackoverflow post\footnote{\url{https://stackoverflow.com/questions/20337029/hashi-puzzle-representation\_-to-solve-all-solutions-with-prolog-restrictions}} given in the assignment. Therefore we use the constraints mentioned in the post but we also add constraints for connectivity. All our constraints are thus:
...
\lstinputlisting[
  language = Prolog-pretty,
  caption  = {pass through constraints},
]{pass_through.pl}
	\item The sum of all bridges coming in/going out from the islands must be equal to the number of the islands.	\lstinputlisting[
  language = Prolog-pretty,
  caption  = {sum constraints},
]{sum.pl}
	\item Bridges cannot cross each other. 
	\lstinputlisting[
  language = Prolog-pretty,
  caption  = {no crossing bridges},
]{no_crossing.pl}
	\item The connectivity constraint is done by checking that all islands can be visited starting from a certain islands. This is done after the search.
	\lstinputlisting[
  language = Prolog-pretty,
  caption  = {islands connected code},
]{connected.pl}
\end{itemize}
All the constraints are active except for the connectivity constraints. The connectivity constraint does not directly make any changes to any of the domains so it a passive constraint. The other constraints make changes to the actual variables. 
\subsection{Optimization}
When one thinks about optimizing hashiwokakero there are some obvious things you could do. Like for example if there is a 4 in one of the four corners, you know that you need to have 2 bridges in the two directions that are left. The same goes for a 6 at one of the sides of the board, since one of it's directions is blocked, you know that there has to be 2 bridges in the 3 leftover directions. For an 8 the only possible thing you can do here is that you need to have 2 bridges in each direction. These seem like good improvements, but actually the eclipse system already did these implicitly for us. This is done by eclipse's domain solving.
\newline

So we felt like these were not really improvements anymore since they were implicitly done. This is why we decided to look for other improvements. These improvements are inspired by \url{http://www.conceptispuzzles.com/index.aspx?uri=puzzle/hashi/techniques}.
...
\begin{enumerate}
    ...
    \label{fig:classic_heuristic}%
\end{figure}
	\item When there is a 3 with three neighbors, under which one neighbor has a 1 and the other neighbor has a two then obviously if you connect one bridge with your 1 neighbor and 2 bridges with the 2 neighbor, you get a isolated set. To prevent this we wrote code so that it automatically connects at least one bridge with the remaining neighbor. The same reasoning can be made for a 2 with two 1's as his neighbor and one other neighbor. If the 2 is connected with both ones we get another isolated set. To prevent this we made sure that the 2 sets at least one bridge to its other neighbor.
	\item Sadly enough we did not find a third improvement, as mentioned before our system already does several improvements implicitly, we looked at the site and we implemented the improvements that we thought were still possible. TODO
\end{enumerate}

As mentioned before the connectivity constraint is done after the search. We select one islands from our islands and then we do a traversal of all it's outgoing bridges to it's neighbors. At the neighbors we check which of their neighbors are still unvisited and we visit them. We continue until we either find that the set is connected or unconnected. The set is connected if we could visit all the islands. Our improvements make sure that some easy cases of isolated sets will never occur. This means that we need to backtrack less. 

\section{CHR}
\subsection{Solver}
\subsubsection{Data representation}
We used the same data representation as for the ECLiPSe solution. The data is stored in board/7 facts with variables representing X,Y, Am, N, E, S, W.  Where Am represents the amount of bridges connected to position (X,Y). N,E,S,W represents the amount of bridges in a certain direction from position (X,Y). 

\subsubsection{Constraints representation}
\begin{itemize}
...
]{chr_bridges_bothways_hashi.pl}
	\item 
		\lstinputlisting[
  language = Prolog-pretty,
  caption  = {INSERT CAPTION},
]{chr_bridges_crossing_hashi.pl}
	\item 
\lstinputlisting[
  language = Prolog-pretty,
  caption  = {INSERT CAPTION},
]{chr_bridges_inside_hashi.pl}
	\item 
\lstinputlisting[
  language = Prolog-pretty,
  caption  = {INSERT CAPTION},
]{chr_directions_equal_hashi.pl}
...
\subsection{Connected constraints}
...
\subsubsection{Impact of improvements}
...
\hline
\end{tabular}
\caption{CHR with connectivity propagator 2}
