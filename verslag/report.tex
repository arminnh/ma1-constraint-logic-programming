...
When we compare the alldifferent from ic\_global vs ic we see that the version from ic\_global is faster! For our viewpoint the increase is not that big but when you look at the channeling column and the classical viewpoint, then you see that the increases in backtracks here are quite big! This was a bit expected though, the alldifferent from ic\_global has been programmed with stronger propagation behaviour \cite{alldiff_global}. 
\subsection{CHR}
