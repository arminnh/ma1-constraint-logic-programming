...
In this report we discuss our different solutions we have created for Sudoku and Hashiwokakero. They are the result of a lot of work and a lot of back tracking on our previously done work. We often came in situations where we got stuck because of the limitations of the ECLiPSe and CHR systems but we also often had to back track on our work since we were often feeling that we were doing things in a non declarative way. We often tried to do things in a procedural way when we first started with Sudoku which means we lost quite some time here since we often had to rethink how we could write things in a more declarative way. For the Hashiwokakero part of the project things went a bit better but we lost here we tried to create a graph representation of the problem but we failed since ECLiPSe doesn't support constraints in conditionals. For this reason we had to retrack our steps back to the structure mentioned on the stackoverflow article.
	\newline
	\newline
...
For the other viewpoint in Sudoku we decided to use the following representation: each number has N positions, represented as tuples (X,Y) (with N the size from one side of the sudoku board). Each of these (X,Y) must be different from each other as well as each X must be different from any other X and Y must be different from all other Y as well. This representation has as a benefit that you know that each value from 1 to N must be in X, so we can already fill this in. The only thing left is to find the right Y.
\newline
\newline
...
Although we mentioned it in the intro of \ref{sub:CHR}, finding out that you should play in that one particular spot was not feasible with our implementation. We did find something else we could do to improve our system.
...
eastermonster & 3.69s & 235.10s & 3.76s \\
...
\begin{table}[h!]
  \begin{tabular}{|c|c|c|c|c|c|c|}
    ...
    & ms & ms & ms \\
    \hline
lambda & 383.27s & 8.07s & 70.85s \\
hard17 & 282.39s & 0.30s & 1.61s \\
eastermonster & 68.83s & 14.74s & 16.73s \\
tarek\_052 & 1.70s & 90.21s & 1.70s \\
goldennugget & 24.11s & 43.1s & 13.81s \\
coloin & 90.63s & 42.73s & 17.73s \\
extra2 & 486.39s & 54.98s & 2.11s \\
extra3 & 385.02s & 7.95s & 70.63s \\
extra4 & 184.18s & 10.60s & 46.23s \\
inkara2012 & 10.75s & 255.18s & 5.50s \\
clue18 & 1068.80s & 569.44s & 249.05s \\
clue17 & 25.60s & 1.01s & 0.98s \\
sudowiki\_nb28 & 1204.91s & 4.58s & 207.25s \\
sudowiki\_nb49 & 130.41s & 21.85s & 70.93s \\
 \hline
  \end{tabular}
  \caption{Sudoku in CHR with heuristics}
\end{table}
When we compare both tables, we see that for our viewpoint we see that there is a good improvement. Although when we look for the classic viewpoint, we see that our heuristic is in most cases quite slower! Although in some cases it does increase it tremendously!
